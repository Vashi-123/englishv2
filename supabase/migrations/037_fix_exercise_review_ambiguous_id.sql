-- Fix Postgres 42702: ambiguous column reference "id" in UPDATE because RETURNS TABLE
-- output columns become plpgsql variables. Qualify table columns with alias in WHERE/SET clauses.

-- Constructor review
DROP FUNCTION IF EXISTS public.apply_constructor_review(BIGINT, INT);
...
BEGIN
  ...
  UPDATE public.user_constructor_cards c
  SET
...
-- Find-the-mistake review
DROP FUNCTION IF EXISTS public.apply_find_mistake_review(BIGINT, INT);
...
BEGIN
  ...
  UPDATE public.user_find_mistake_cards c
  SET
    review_count = c.review_count + 1,
    correct_count = c.correct_count + CASE WHEN q >= 3 THEN 1 ELSE 0 END,
    wrong_count = c.wrong_count + CASE WHEN q < 3 THEN 1 ELSE 0 END,
